...
echo Starting deployment of Maps...
...
# 4. Install npm and build packages for react-app directory
if [ -e "$DEPLOYMENT_TARGET/react-app/package.json" ]; then
  cd "$DEPLOYMENT_TARGET/react-app"
  echo "Running $NPM_CMD install --no-optional --silent --production for react-app directory"
  eval $NPM_CMD install --no-optional --silent --production
  ##################
  echo Creating config file for build process...
